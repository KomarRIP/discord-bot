...
function stepIndex(stepKey) {
    const order = ["template", "unit_name", "unit_size", "intake_mode", "guest_policy", "preview"];
...
export function buildSetupWizardMessage(params) {
    const { state } = params;
    ...
    const row = new ActionRowBuilder().addComponents(backBtn, editBtn, nextBtn, cancelBtn);
    return { embeds: [embed], components: [row] };
}
function formatCounts(c) {
    return `создать: ${c.create}, обновить: ${c.update}, пропустить: ${c.skip}`;
}
export function buildSetupWizardPreviewMessage(params) {
    ...
    const fields = [
        ...
        {
            name: "Summary",
            value: `**roles**: ${formatCounts(params.preview.summary.roles)}\n` +
    ...
    const prevBtn = new ButtonBuilder()
        .setCustomId(encodeCustomId({ ns: "wizard", action: "preview", version: "v1", sessionId: params.state.sessionId, page: Math.max(1, safePage - 1) }))
        .setLabel("Назад")
        .setStyle(ButtonStyle.Secondary)
    ...
    const cancelBtn = new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(params.state.status === "deploying");
    const row1 = new ActionRowBuilder().addComponents(prevBtn, nextBtn);
    const row2 = new ActionRowBuilder().addComponents(backWizardBtn, confirmBtn, cancelBtn);
    return { embeds: [embed], components: [row1, row2] };
}
export function buildSetupWizardModal(params) {
    ...
    modal.addComponents(new ActionRowBuilder().addComponents(size), new ActionRowBuilder().addComponents(limit));
    return modal;
}
