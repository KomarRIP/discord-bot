...
export function buildDeployPreviewMessage(params) {
    ...
    const nextBtn = new ButtonBuilder()
        .setCustomId(encodeCustomId({ ns: "deploy", action: "preview", version: "v1", page: Math.min(totalPages, safePage + 1) }))
        .setLabel("Далее")
        .setStyle(ButtonStyle.Secondary)
    ...
    const cancelBtn = new ButtonBuilder()
        ...
        .setLabel("Cancel")
        .setStyle(ButtonStyle.Secondary);
    const row = new ActionRowBuilder().addComponents(prevBtn, nextBtn, applyBtn, cancelBtn);
    return {
        embeds: [embed],
        components: [row],
    };
