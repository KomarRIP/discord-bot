...
export class TemplateDeploymentService {
    ...
    async buildPlan(ctx, template, configHash) {
        ...
        const msgIntake = "Чтобы подать заявку на вступление, используйте `/intake apply`.\n" +
    ...
    async executePlan(ctx, template, deploymentId, plan, requestCtx) {
        ...
        for (const step of plan.filter((p) => p.scope === "message")) {
            ...
            const channelId = channelIdByKey.get(msg.channelKey) ??
                this.deps.storage.mappings.getMapping(ctx.guildId, "channel", msg.channelKey)?.discordId;
            if (!channelId)
                throw new AppError({ code: "NOT_FOUND", message: `Channel not found for message: ${msg.channelKey}` });
            ...
            try {
                const res = await this.deps.queue.enqueue({
                    ...
                    idempotencyKey: step.idempotencyKey,
                    budget: { deadlineAt, maxAttempts: 8 },
                    execute: async () => this.deps.discord.ensureMessage({
                        guildId: ctx.guildId,
                        channelId,
                        messageKey: step.key,
                        content: msg.content,
