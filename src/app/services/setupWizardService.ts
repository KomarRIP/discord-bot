...
export type SetupWizardStateDto = {
  sessionId: string;
  guildId: string;
...
export type SetupWizardUiDto = { kind: "wizard"; state: SetupWizardStateDto };

const STEP_ORDER: SetupWizardStepKey[] = ["template", "unit_name", "unit_size", "intake_mode", "guest_policy", "preview"];

export class SetupWizardService {
  constructor(private readonly storage: Storage) {}

  async start(ctx: CommandContextDto): Promise<ResultDto<{ ui: SetupWizardUiDto }>> {
    if (!ctx.guildId) {
      return {
        type: "error",
  ...
  async status(ctx: CommandContextDto): Promise<ResultDto<{ ui?: SetupWizardUiDto; sessionId?: string }>> {
    const active = this.storage.setupSessions.getActiveSession(ctx.guildId);
    if (!active) {
      return {
        type: "success",
  ...
  async navigate(ctx: CommandContextDto, input: { sessionId: string; dir: "back" | "next" }): Promise<ResultDto<{ ui: SetupWizardUiDto }>> {
    if (!ctx.guildId) {
    ...
    if (session.status === "deploying") throw new AppError({ code: "CONFLICT", message: "Сессия уже в deploying." });

    const current = session.stepKey as SetupWizardStepKey;
    ...
    if (input.dir === "next") this.validateStepOrThrow(current, answers);
  ...
  async updateAnswersFromModal(
    ctx: CommandContextDto,
    input: { sessionId: string; field: "unit_name" | "unit_size"; unitName?: string; unitSize?: number; positionsLimitPerMember?: number },
  ): Promise<ResultDto<{ ui: SetupWizardUiDto }>> {
    if (!ctx.guildId) {
      return { type: "error", errorCode: "VALIDATION_FAILED", userMessage: "guildId обязателен.", retryable: false };
    }
    const session = this.loadActiveSessionOrThrow(ctx.guildId, input.sessionId);
  ...
  markDeploying(guildId: string, sessionId: string): void {
  ...
  private toStateDto(session: {
    sessionId: string;
    guildId: string;
    status: SetupWizardStateDto["status"];
    stepKey: string;
    answersJson: string;
    expiresAt: string;
  }): SetupWizardStateDto {
    const answers = this.safeParseAnswers(session.answersJson);
    const stepKey = (STEP_ORDER.includes(session.stepKey as SetupWizardStepKey) ? session.stepKey : "template") as SetupWizardStepKey;
    return {
  ...
  private safeParseAnswers(raw: string): SetupAnswers {
    try {
      const parsed = JSON.parse(raw) as Partial<SetupAnswers>;
  ...
  private validateStepOrThrow(stepKey: SetupWizardStepKey, answers: SetupAnswers): void {
    if (stepKey === "template") {
      if (!answers.templateId) throw new AppError({ code: "VALIDATION_FAILED", message: "Не выбран шаблон.", retryable: false });
      return;
    }
  ...
  private loadActiveSessionOrThrow(guildId: string, sessionId: string) {
    const active = this.storage.setupSessions.getActiveSession(guildId);
    if (!active) throw new AppError({ code: "NOT_FOUND", message: "Активная setup-сессия не найдена.", retryable: false });
