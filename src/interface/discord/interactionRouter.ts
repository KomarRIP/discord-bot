...
import type { ButtonInteraction, ChatInputCommandInteraction, Client, Interaction } from "discord.js";
...
export function createInteractionRouter(params: {
  ...
  async function mustBeGuild(interaction: ChatInputCommandInteraction | ButtonInteraction) {
    if (!interaction.inGuild() || !interaction.guildId) {
      throw new AppError({ code: "VALIDATION_FAILED", message: "Команда доступна только в guild.", retryable: false });
    }
  }

  async function mustBeOwnerOrBotAdmin(interaction: ChatInputCommandInteraction | ButtonInteraction) {
